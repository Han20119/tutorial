...
class Homework implements TreeWillExpandListener,TreeSelectionListener{
	private JFrame frame = new JFrame("파일 탐색기");
 	private Container con = null;

 	private JSplitPane AMain=new JSplitPane();
 	private JScrollPane ALeft=null;
 	private JPanel ARight=new JPanel(new BorderLayout());
 
 	private DefaultMutableTreeNode root = new DefaultMutableTreeNode("내컴퓨터");
 	private JTree tree;
 
 	private JPanel ANorth=new JPanel();
 	...
 	void init(){
		...
		ANorth.add(northText);
		con.add(ANorth,"North");
		File file=new File("");
		...
		tree=new JTree(root);
		ALeft=new JScrollPane(tree);
	  
		AMain.setDividerLocation(150);
		AMain.setLeftComponent(ALeft);
		AMain.setRightComponent(ARight);
		con.add(AMain);
	}
 ...
 public void valueChanged(TreeSelectionEvent e){
	 if(((String)((DefaultMutableTreeNode)e.getPath().getLastPathComponent()).getUserObject()).equals("내컴퓨터")){
		 ...
		 try{
			 ...
			 ARight=new FView(getPath(e)).getTablePanel();
			 AMain.setRightComponent(ARight);
		 }
	  catch(Exception ex){
		  JOptionPane.showMessageDialog(frame, "디스크 혹은 파일을 찾을수 없습니다.");
